...
class DeafGeneSystemApp:
    ...
    def setup_application(self):
        ...
        print(f"开始初始化应用...", file=sys.stderr)
        ...
        if icon_path.exists():
            ...
            print(f"图标文件不存在: {icon_path}", file=sys.stderr)
        
        self.setup_styles()
        
        elapsed = __import__('time').time() - self._start_time
        print(f"应用初始化完成，耗时: {elapsed:.2f}s", file=sys.stderr)
    ...
    def show_main_window(self):
        ...
        print(f"创建主窗口...", file=sys.stderr)
        
        self.main_window = MainWindow()
        self.main_window.show()
        
        elapsed = __import__('time').time() - self._start_time
        print(f"主窗口显示完成，总耗时: {elapsed:.2f}s", file=sys.stderr)
...
def main():
    ...
    print(f"============== 程序启动 ==============", file=sys.stderr)
    print(f"项目目录: {project_root}", file=sys.stderr)
